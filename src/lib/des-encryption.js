  ...
  if (leng > 0) {
    if (leng < 4) {
      ...
      for (let i = 0; i < iterator; i++) {
        const tempData = data.substring(i * 4, i * 4 + 4);
      ...
      if (remainder > 0) {
        const remainderData = data.substring(iterator * 4, leng);
...
function getKeyBytes(key) {
  ...
  if (remainder > 0) {
    keyBytes[iterator] = strToBt(key.substring(iterator * 4, leng));
  }
  return keyBytes;
}
...
function sBoxPermute(expandByte) {
  ...
  for (m = 0; m < 8; m++) {
    ...
    switch (m) {
      case 0: binary = getBoxBinary(s1[i][j]); break;
      case 1: binary = getBoxBinary(s2[i][j]); break;
      case 2: binary = getBoxBinary(s3[i][j]); break;
      case 3: binary = getBoxBinary(s4[i][j]); break;
      case 4: binary = getBoxBinary(s5[i][j]); break;
      case 5: binary = getBoxBinary(s6[i][j]); break;
      case 6: binary = getBoxBinary(s7[i][j]); break;
      case 7: binary = getBoxBinary(s8[i][j]); break;
    }
